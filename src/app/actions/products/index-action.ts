  ...
  export class Action extends ActionBase<Store, Event> {
    protected initialize() {
      ...
      this.observe('search').subscribe(async condition => {
        if (condition) {
          this.store.condition = Object.assign(this.store.condition, condition);
        }
        this.next('render');
        this.setStore(
      ...
      this.observe('remove').subscribe(() => {
        const ids = this.store.items.filter(x => x.isSelected).map(x => x.id);
        const text = ids.length + '件の製品情報を削除します。よろしいですか？';
