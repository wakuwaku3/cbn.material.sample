...
namespace InnerScope {
  export const getHeight = (theme: Theme) => {
    if (window.innerWidth >= theme.breakpoints.values.sm) {
      const sm = theme.mixins.toolbar[
        theme.breakpoints.up('sm')
      ] as React.CSSProperties;
      return sm ? Number(sm.minHeight) : 0;
    }
    return Number(theme.mixins.toolbar.minHeight);
  };
  interface Style {
    bar;
    title;
  ...
  const Inner = decorate(style)(
    class extends StyledComponentBase<Style, InnerProps, State> {
      ...
      public getElement = ({ history }: RouteComponentProps<{}>) => {
        return (
              ...
              {this.props.authenticated && (
                <div>
                  ...
                  >
                    <AppMenuItem
                      onClick={() => {
                        history.push(Url.productsSimpleIndex);
                    ...
                    <AppMenuItem
                      onClick={() => {
                        history.push(Url.productsPaneIndex);
                        this.handleCloseMenu();
                      }}
                    >
                      Pane
                    ...
                    <AppMenuItem
                      onClick={() => {
                        history.push(Url.productsWindowIndex);
                        this.handleCloseMenu();
                      }}
                    >
                      Window
